...
def run_main(input_filename):
    """
    Function to run main(input_filename) in a separate thread.
    """
    input_filename = "input.txt"  # Assuming input_filename is defined here
    main(input_filename)  # Assuming main function is defined somewhere
    print("done")

if __name__ == "__main__":
    # Start FastAPI application
    uvicorn_thread = ThreadPoolExecutor(max_workers=1)
    uvicorn_thread.submit(uvicorn.run, app, host="0.0.0.0", port=8000)

    # Start main(input_filename) in a separate thread
    main_thread = ThreadPoolExecutor(max_workers=1)
    main_thread.submit(run_main, "input.txt")
